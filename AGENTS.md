...
在创建、修改或删除任何自动化测试前，必须阅读并遵守根目录 [AI_TEST_POLICY.md](AI_TEST_POLICY.md)。默认不新增或调整测试；只有该政策定义的必要场景成立，或用户明确要求时，才可变更测试。测试必要性不明确时，不要自行补测，改为执行与变更相匹配的最小验证，并在最终说明中注明未新增测试的依据。
