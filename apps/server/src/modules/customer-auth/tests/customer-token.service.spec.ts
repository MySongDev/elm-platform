...
import { CustomerTokenService } from '@/modules/customer-auth/customer-token.service'
...
describe('customerTokenService', () => {
  function createService() {
    ...
    const redis = {
      ...
      getSetMembers: jest.fn(async (key: string) => Array.from(userSets.get(key) ?? [])),
      expire: jest.fn(),
    } as any
  ...
  it('rotates refresh tokens by consuming the old active token', async () => {
    ...
    expect(redis.removeFromSet).toHaveBeenCalledWith('customer:refresh:user:7', tokenId)
  })
  ...
  it('revokes all sessions when a used refresh token is replayed', async () => {
    ...
    expect(redis.del).toHaveBeenCalledWith(`customer:refresh:${secondTokenId}`)
    expect(redis.del).toHaveBeenCalledWith(`customer:refresh:used:${firstTokenId}`)
    expect(redis.del).toHaveBeenCalledWith('customer:refresh:user:7')
  })
})
