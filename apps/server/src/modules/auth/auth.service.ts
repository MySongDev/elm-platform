...
const DEFAULT_SESSION_EXPIRES_IN = '24h'
const DEFAULT_SESSION_TTL_SECONDS = 24 * 60 * 60
const REMEMBER_ME_SESSION_EXPIRES_IN = '7d'
const REMEMBER_ME_SESSION_TTL_SECONDS = 7 * DEFAULT_SESSION_TTL_SECONDS
...
export class AuthService {
  ...
  async getUserMenus(userId: number): Promise<MenuNode[]> {
    ...
    const userPermissions = await this.resolveEffectivePermissions(user)
  ...
  async login(account: string | undefined, password: string, ip?: string, userAgent?: string, rememberMe = false) {
    if (!account) {
  ...
  async getProfile(userId: number) {
    ...
    return {
      ...user,
      permissions: await this.resolveEffectivePermissions(user),
    }
  }
