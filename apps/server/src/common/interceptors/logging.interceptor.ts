...
export class LoggingInterceptor implements NestInterceptor {
  ...
  private resolveOperationMeta(method: string, path: string) {
    ...
    if (normalizedPath.includes('/admin/system/roles')) {
      return {
        module: '系统管理',
        action: this.resolveCrudAction(method, '角色'),
      }
    }
    if (normalizedPath.includes('/admin/system/menus')) {
      return {
        module: '系统管理',
        action: this.resolveCrudAction(method, '菜单'),
      }
    }
    if (normalizedPath.includes('/admin/system/depts')) {
      return {
        module: '系统管理',
        action: this.resolveCrudAction(method, '部门'),
      }
    }
    if (normalizedPath.includes('/users')) {
      return {
        module: '用户管理',
        action: this.resolveCrudAction(method, '用户'),
      }
    }
    if (normalizedPath.includes('/admin/commerce/restaurants')) {
      return {
        module: '业务管理',
        action: this.resolveCrudAction(method, '商家'),
      }
    }
    if (normalizedPath.includes('/admin/commerce/foods')) {
      return {
        module: '业务管理',
        action: this.resolveCrudAction(method, '商品'),
      }
    }
    if (normalizedPath.includes('/admin/commerce/orders')) {
      return {
        module: '业务管理',
        action: this.resolveCrudAction(method, '订单'),
      }
    }

    return {
      module: '后台管理',
      action: this.resolveMethodAction(method),
    }
  }
