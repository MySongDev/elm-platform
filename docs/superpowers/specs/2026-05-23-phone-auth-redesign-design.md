...
本次目标是重做认证模块边界，同时改造后台管理端和用户端登录流程：后台管理员支持用户名或手机号加密码登录；普通用户使用独立账号表，支持手机号密码、短信验证码、验证码注册和验证码登录。短信发送先�?provider 抽象，开发环境使�?mock，后续可接阿里云或腾讯云短信�?

## 目标

- 后台管理员和普通用户身份彻底分离，避免普通用�?token 访问后台权限接口�?
- 后台管理员只支持用户�?手机�?+ 密码登录，不支持短信验证码登录�?
- 保留默认可登录管理员：`admin / admin123`，并绑定手机�?`13800138000`，因此也可用 `13800138000 / admin123` 登录�?
- 普通用户新增独立数据模型，不复用后�?`users` 表�?
- 普通用户支持：验证码快捷注册、验证码 + 密码注册、手机号 + 密码登录、手机号 + 验证码登录�?
- 用户端验证码登录时，手机号不存在的行为由配置控制，默认自动注册并登录，后续可切换为必须先注册�?
- 短信验证码使�?Redis 存储和限流，开发环�?mock，生产环境禁�?mock provider 误上线�?
- 后台登录页和用户端登�?注册页都接入新认证流程�?

## 非目�?

- 不在本期接入真实短信厂商；仅提供 provider 接口�?mock 实现�?
- 不重做后台菜单、角色、按钮权限体系，只调�?JWT 身份边界和登录入口�?
- 不把后台管理员迁移到新表；现�?`users` 表继续作为后台管理员表�?
...
1. **Admin Auth**
   - 使用现有 `User`/`users` 表�?
   - 登录账号字段�?`account`，可匹配 `username` �?`phone`�?
   - 登录成功签发 `subjectType: 'admin'` �?JWT�?
   - `GET /auth/profile`、`GET /auth/menus` 等后台接口只接受 admin token�?

2. **Customer Auth**
   - 新增普通用户表，例�?`CustomerUser`�?
   - 登录和注册均以手机号为主身份�?
   - 登录成功签发 `subjectType: 'customer'` �?JWT�?
   - 用户�?profile 和后续用户域接口只接�?customer token�?
...
`phone` 已有唯一约束，管理员登录时可通过 `username` �?`phone` 查找同一条记录�?
...
`password` 可为空：验证码快捷注册会创建无密码账号；验证�?+ 密码注册会保�?bcrypt hash。密码登录只允许已设置密码的普通用户�?
...
- 后续阿里�?腾讯�?provider 实现同一接口�?

Redis key 设计�?

- 验证码：`sms:code:{scene}:{phone}`，TTL 5 分钟�?
- 发送冷却：`sms:cooldown:{phone}`，TTL 60 秒�?
- 每日次数：`sms:daily:{phone}:{yyyyMMdd}`，每日最�?10 次�?

验证码建议存�?hash 或包�?hash 的结构体，验证成功后删除验证�?key，避免重复使用�?
...
生产环境如果仍配�?`SMS_PROVIDER=mock`，发送验证码时必须报错；如果后续统一做配置校验，也可在应用启动阶段直接失败�?
...
- �?`account` 匹配 `username` �?`phone`�?
- 账号不存在或密码错误统一返回“账号或密码错误”�?
- 账号禁用返回“账号已被禁用”�?
- 成功后记录登录日志、在线用户状态，并返回现有后�?token/user 结构�?
- token payload �?`subjectType: 'admin'`�?

兼容策略：前端改�?`account`；后端可以在短期内接受旧字段 `username` 作为 `account` 来源，减少联调成本�?
...
- 校验手机号格式�?
- 检�?60 秒冷却和每日 10 次限制�?
- 生成验证码，写入 Redis，调用短�?provider�?
- 开发环�?mock 可在响应中返�?`debugCode` 或在服务端日志输出�?
...
- 校验验证码�?
- 手机号已注册则返回冲突错误�?
- 创建普通用户；如果传入 password，保�?bcrypt hash�?
- 可选择注册后直接返�?token/profile，减少用户端二次登录�?
...
- 成功后更�?`lastLoginAt`，返�?customer token/profile�?
...
- 校验验证码�?
- 如果手机号不存在�?`CUSTOMER_LOGIN_AUTO_REGISTER=true`，自动创建普通用户并登录�?
- 如果手机号不存在且自动注册关闭，返回“手机号未注册，请先注册”�?
- 成功后更�?`lastLoginAt`，返�?customer token/profile�?

#### `GET /customer-auth/profile`

返回当前普通用�?profile，只接受 customer token�?
...
- 默认提示保留：`admin / admin123`，并可提�?`13800138000 / admin123`�?
...
用户端登录成功后保存普通用�?token/profile。后续用户端 API 通过 customer token 访问用户域接口�?
...
- 普通用户账号禁用：返回“账号已被禁用”�?
- customer token 访问后台接口：返�?401 �?403�?
- admin token 访问 customer profile：返�?401 �?403�?
