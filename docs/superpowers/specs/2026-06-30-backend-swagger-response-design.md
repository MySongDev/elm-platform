...
3. 按接口真实场景补齐 `400/401/403/404/409/429/500` 错误响应。
...
- `code`: 业务成功码，当前固定为 `200`；它与 POST 路由可能返回的 HTTP `201` 相互独立。
- `message`: 成功提示。
...
- `ApiEmptyResponse(options?)`: 描述业务返回值为 `undefined`、JSON 中不含 `data` 的统一响应。
...
10. 触发发送频率限制的接口声明 `429`。
11. `500` 使用统一错误 Schema，作为未预期服务异常说明。
